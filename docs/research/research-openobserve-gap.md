...
OpenObserve is the closest full product to Krönika's storage layer: an
...
clarifies what Krönika deliberately is (a compliance-grade resilience
...
**AGPL-3.0**; Krönika is Apache-2.0. Ideas, architectures and feature
...
Krönika ships in this area is **clean-room**, implemented against DuckDB's
...
What Krönika already has that OpenObserve does not:
...
What OpenObserve has that Krönika lacks (the honest gap):

| Capability | OpenObserve | Krönika | Priority read |
| --- | --- | --- | --- |
...
1. **Parquet export + retention/compaction.** A hot embedded store with no
   ...
   operational gap in Krönika. The idea to borrow is the *shape*: hot rows
...
2. **Immutability as a feature, not a limitation.** OpenObserve's lake
   ...
   Krönika reframes the same property as *compliance evidence*: parquet
...
3. **WAL / durability documentation.** OpenObserve documents its write-ahead
   behaviour as an operator-facing guarantee. Krönika's equivalent
...
The dashboard builder (Krönika's opinionated fixed pages are the product),
...
PromQL (Krönika's semantic layer is SQL-over-YAML by design). Alerting and
