...
OpenObserve is the closest full product to Tumult's analytics store: an
...
clarifies what the analytics layer deliberately is (a compliance-grade resilience
...
**AGPL-3.0**; Tumult is Apache-2.0. Ideas, architectures and feature
...
Tumult ships in this area is **clean-room**, implemented against DuckDB's
...
What Tumult already has that OpenObserve does not:
...
What OpenObserve has that Tumult lacks (the honest gap):

| Capability | OpenObserve | Tumult | Priority read |
| --- | --- | --- | --- |
...
1. **Parquet export + retention/compaction.** A hot embedded store with no
   ...
   operational gap in the analytics store. The idea to borrow is the *shape*: hot rows
...
2. **Immutability as a feature, not a limitation.** OpenObserve's lake
   ...
   the analytics layer reframes the same property as *compliance evidence*: parquet
...
3. **WAL / durability documentation.** OpenObserve documents its write-ahead
   behaviour as an operator-facing guarantee. Our equivalent
...
The dashboard builder (the opinionated fixed pages are the product),
...
PromQL (the semantic layer is SQL-over-YAML by design). Alerting and
