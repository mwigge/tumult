...
pub fn begin_connect(host: &str, port: u16, auth_method: &str) -> SpanGuard {
...
pub fn begin_execute(command: &str, timeout_s: Option<f64>) -> SpanGuard {
    let tracer = global::tracer(TRACER);
...
pub fn begin_upload(remote_path: &str, file_bytes: u64) -> SpanGuard {
    let tracer = global::tracer(TRACER);
    let span = tracer
...
pub fn event_command_completed(exit_code: i64, stdout_bytes: usize, stderr_bytes: usize) {
...
pub fn event_upload_completed(bytes: u64) {
    let cx = opentelemetry::Context::current();
    cx.span().add_event(
...
pub fn event_auth_success(method: &str) {
    let cx = opentelemetry::Context::current();
    cx.span().add_event(
        "ssh.auth.success",
