//! Declared-topology persistence and the edge/node readbacks that feed
//! lineage and recommendation computation. Executor only — the model, SQL
//! and derivations live in `tumult-graph`.

use duckdb::params;
use tumult_graph::{sql, EdgeRecord, GraphDelta, TOPOLOGY_RUN_ID};
...
impl AnalyticsStore {
    ...
    pub fn graph_edges_by_rels(&self, rels: &[&str]) -> Result<Vec<EdgeRecord>, AnalyticsError> {
        if rels.is_empty() {
            return Ok(Vec::new());
        }
    ...
    pub fn graph_nodes_with_attrs(&self, kind: &str) -> Result<Vec<NodeAttrs>, AnalyticsError> {
        let mut stmt = self.conn.prepare(sql::NODE_ATTRS_BY_KIND)?;
        let rows = stmt
...
mod tests {
    ...
    fn import_is_idempotent_and_traversable() {
        ...
        let edges = s.graph_edges_by_rels(&["depends_on"]).unwrap();
        assert_eq!(edges.len(), 2);

        let ego = s
            .graph_neighbors("svc:api", Some("depends_on"), 1)
            .unwrap()
    ...
    fn empty_rels_returns_empty() {
        let s = AnalyticsStore::in_memory().unwrap();
        assert!(s.graph_edges_by_rels(&[]).unwrap().is_empty());
    }
}
