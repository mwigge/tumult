...
pub fn journal_to_experiment_batch(journal: &Journal) -> Result<RecordBatch, AnalyticsError> {
    ...
    let hyp_before_met = journal.steady_state_before.as_ref().map(|h| h.met);
    let hyp_after_met = journal.steady_state_after.as_ref().map(|h| h.met);
    let estimate_accuracy = journal.analysis.as_ref().and_then(|a| a.estimate_accuracy);
    let resilience_score = journal.analysis.as_ref().and_then(|a| a.resilience_score);
    ...
    Ok(RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(BooleanArray::from(vec![hyp_before_met])),
            Arc::new(BooleanArray::from(vec![hyp_after_met])),
            Arc::new(Float64Array::from(vec![estimate_accuracy])),
            Arc::new(Float64Array::from(vec![resilience_score])),
        ],
    )?)
}
...
mod tests {
    ...
    fn activity_batch_method() {
        ...
        let names = batch
            ...
            .downcast_ref::<StringArray>()
            .unwrap();
        assert_eq!(names.value(0), "kill-connections");
    }
